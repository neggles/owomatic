...
class General(commands.Cog, name="general"):
    ...
    async def botinfo(self, ctx: ApplicationCommandInteraction) -> None:
        ...
        """
        icon_url = ctx.author.display_avatar.url if ctx.author.display_avatar else ctx.author.avatar.url
        ...
        embed.add_field(name="Source repo:", value=self.bot.config.get("repo_url", "N/A"), inline=True)
        embed.add_field(name="Running on", value=f"Python {platform.python_version()}", inline=False)
        embed.add_field(name="Timezone:", value=str(self.bot.config.get("timezone", "Unknown")), inline=True)
